...
3. **Тестировщик — Qwen 397B**: анализирует код, запускает проверки, может использовать Playwright, циклически отправляет ошибки программисту и деплоит на Render.
...
Деплой выполняется через Render API при наличии `RENDER_API_KEY`, `RENDER_OWNER_ID` и настроек сервиса. Если ключи не заданы, тестировщик честно отмечает деплой как пропущенный и продолжает локальную проверку.
