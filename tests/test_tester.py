...
from autocoder_agent.core.llm import OpenRouterClient


def test_tester_exercises_generated_crud_endpoints(tmp_path, monkeypatch):
...
def test_render_blueprint_uses_project_run_command(tmp_path):
    ...
    tester = AutocoderTester(OpenRouterClient(api_key=None, base_url=config.openrouter_base_url), config.qwen_model, config)
